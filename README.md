# MiniMarket
Realizar un Mini Market con las funciones necesarias 
Una tienda online realizada con Python es una aplicación web que permite vender productos o servicios a través de internet, desarrollada utilizando este lenguaje por su potencia y versatilidad. Que facilitan la creación de sistemas de registro de usuarios, carritos de compra y gestión de productos.

Python ofrece una estructura clara, seguridad y fácil mantenimiento, lo que permite desarrollar una tienda funcional y escalable. Este tipo de proyecto es ideal para aprender desarrollo web, gestión de bases de datos y programación orientada a objetos, además de ser una buena base para negocios pequeños que buscan digitalizar sus ventas.
